...
3. 进入 `Models` 页面，了解 `vox-lite-v1` 和 `vox-prime-v1` 的差异。
...
- 生产环境建议固定使用稳定模型版本，例如 `vox-lite-v1` 或 `vox-prime-v1`。
...
5. 如果需要品牌或个人声音，进入 `Clone` 页面。
6. 在 Clone 页面上传样本、确认授权，页面会调用真实上传和克隆接口，生成新的 `voice_id`。
7. 如果克隆状态尚未就绪，可用 `Refresh status` 主动拉取服务端状态；失败后可点 `Retry` 重试。
8. 如果不再需要该克隆音色，可用 `Delete voice` 调用真实删除接口。
9. 如果没有真实样本，进入 `Design` 页面，通过 prompt 生成真实音色预览。
10. 在 Design 页面点击 `Save as Voice`，系统会把预览音频再走一次上传+克隆，固化成可复用 `voice_id`。
...
| --- | --- |
| `Home` | 官网首页和能力概览 |
| `Playground` | TTS 试音、调参、真实合成、复制 API 请求 |
| `Models` | 查看 VoxLite / VoxPrime 能力差异和复制 model id |
| `Voices` | 实时声音库、搜索筛选、试听、收藏、复制 voice_id |
| `Clone` | 上传样本、真实调用上传与克隆接口、刷新状态、失败重试、删除音色 |
| `Design` | 通过 prompt 生成真实预览、保存为新 voice_id、刷新状态、失败重试、删除音色 |
...
- `Usage / Logs / Pricing / Safety / Team` 主要还是产品原型视图，不直接读取后台管理接口。
- `WebSocket` 页面内容仍是信息架构，尚未接真实流式协议。
- `VoxLite / VoxPrime` 目前是你们产品侧的模型分层命名；这次实际接入的底层推理接口是 MOSI 的 `moss-tts` 与 `moss-voice-generator`。
